-- Ward Watch - Supabase Database Schema with Row Level Security (RLS)
-- This schema includes comprehensive RLS policies for secure multi-tenant access
...
ALTER TABLE patient_staff_assignments ENABLE ROW LEVEL SECURITY;

-- ========================================
-- HELPER FUNCTIONS FOR RLS
-- ========================================

-- Function to get current admin ID from JWT
CREATE OR REPLACE FUNCTION auth.current_admin_id()
RETURNS UUID AS $$
...
CREATE POLICY "Admins can view their own data"
  ...
  USING (admin_id = auth.current_admin_id());
...
-- ========================================
-- RLS POLICIES FOR MASTER DATA (Read-Only for Authenticated Admins)
-- ========================================

CREATE POLICY "Authenticated admins can view departments"
  ON departments FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

CREATE POLICY "Authenticated admins can view staff"
  ON staff FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

CREATE POLICY "Authenticated admins can view medications"
  ON medications FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

CREATE POLICY "Authenticated admins can view routes"
  ON routes_of_administration FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

CREATE POLICY "Authenticated admins can view procedures"
  ON procedures FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

CREATE POLICY "Authenticated admins can view investigations"
...
-- Authenticated admins can view all patients
CREATE POLICY "Authenticated admins can view patients"
  ON patients FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can create patients
CREATE POLICY "Authenticated admins can create patients"
  ON patients FOR INSERT
  WITH CHECK (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can update patients
CREATE POLICY "Authenticated admins can update patients"
...
-- Authenticated admins can view medication orders
CREATE POLICY "Authenticated admins can view medication orders"
  ON medication_orders FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can create medication orders
CREATE POLICY "Authenticated admins can create medication orders"
  ON medication_orders FOR INSERT
  WITH CHECK (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can update medication orders
CREATE POLICY "Authenticated admins can update medication orders"
  ON medication_orders FOR UPDATE
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can delete medication orders
CREATE POLICY "Authenticated admins can delete medication orders"
...
-- Authenticated admins can view procedure orders
CREATE POLICY "Authenticated admins can view procedure orders"
  ON procedure_orders FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can create procedure orders
CREATE POLICY "Authenticated admins can create procedure orders"
  ON procedure_orders FOR INSERT
  WITH CHECK (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can update procedure orders
CREATE POLICY "Authenticated admins can update procedure orders"
  ON procedure_orders FOR UPDATE
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can delete procedure orders
CREATE POLICY "Authenticated admins can delete procedure orders"
  ON procedure_orders FOR DELETE
  USING (auth.current_admin_id() IS NOT NULL);

-- ========================================
-- RLS POLICIES FOR INVESTIGATION ORDERS
-- ========================================

-- Authenticated admins can view investigation orders
CREATE POLICY "Authenticated admins can view investigation orders"
  ON investigation_orders FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can create investigation orders
CREATE POLICY "Authenticated admins can create investigation orders"
  ON investigation_orders FOR INSERT
  WITH CHECK (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can update investigation orders
CREATE POLICY "Authenticated admins can update investigation orders"
  ON investigation_orders FOR UPDATE
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can delete investigation orders
CREATE POLICY "Authenticated admins can delete investigation orders"
  ON investigation_orders FOR DELETE
  USING (auth.current_admin_id() IS NOT NULL);

-- ========================================
-- RLS POLICIES FOR NURSING NOTES
-- ========================================

-- Authenticated admins can view nursing notes
CREATE POLICY "Authenticated admins can view nursing notes"
  ON nursing_notes FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can create nursing notes
CREATE POLICY "Authenticated admins can create nursing notes"
  ON nursing_notes FOR INSERT
  WITH CHECK (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can update nursing notes
CREATE POLICY "Authenticated admins can update nursing notes"
  ON nursing_notes FOR UPDATE
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can delete nursing notes
CREATE POLICY "Authenticated admins can delete nursing notes"
  ON nursing_notes FOR DELETE
  USING (auth.current_admin_id() IS NOT NULL);

-- ========================================
-- RLS POLICIES FOR VITAL SIGNS
-- ========================================

-- Authenticated admins can view vital signs
CREATE POLICY "Authenticated admins can view vital signs"
  ON vital_signs FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can create vital signs
CREATE POLICY "Authenticated admins can create vital signs"
  ON vital_signs FOR INSERT
  WITH CHECK (auth.current_admin_id() IS NOT NULL);

-- ========================================
-- RLS POLICIES FOR TREATMENT HISTORY
-- ========================================

-- Authenticated admins can view treatment history
CREATE POLICY "Authenticated admins can view treatment history"
  ON treatment_history FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can create treatment history
CREATE POLICY "Authenticated admins can create treatment history"
  ON treatment_history FOR INSERT
  WITH CHECK (auth.current_admin_id() IS NOT NULL);
...
-- Authenticated admins can view notifications
CREATE POLICY "Authenticated admins can view notifications"
  ON notification_queue FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can create notifications
CREATE POLICY "Authenticated admins can create notifications"
  ON notification_queue FOR INSERT
  WITH CHECK (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can update notifications
CREATE POLICY "Authenticated admins can update notifications"
  ON notification_queue FOR UPDATE
  USING (auth.current_admin_id() IS NOT NULL);

-- ========================================
-- RLS POLICIES FOR PATIENT STAFF ASSIGNMENTS
-- ========================================

-- Authenticated admins can view patient staff assignments
CREATE POLICY "Authenticated admins can view patient staff assignments"
  ON patient_staff_assignments FOR SELECT
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can create patient staff assignments
CREATE POLICY "Authenticated admins can create patient staff assignments"
  ON patient_staff_assignments FOR INSERT
  WITH CHECK (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can update patient staff assignments
CREATE POLICY "Authenticated admins can update patient staff assignments"
  ON patient_staff_assignments FOR UPDATE
  USING (auth.current_admin_id() IS NOT NULL);

-- Authenticated admins can delete patient staff assignments
CREATE POLICY "Authenticated admins can delete patient staff assignments"
  ON patient_staff_assignments FOR DELETE
  USING (auth.current_admin_id() IS NOT NULL);
