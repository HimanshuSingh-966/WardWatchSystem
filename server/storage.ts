...
export class SupabaseStorage implements IStorage {
  ...
  async dischargePatient(id: string): Promise<Patient> {
    ...
    // Deactivate all active orders for this patient
    await Promise.all([
      supabase.from('medication_orders').update({ is_active: false }).eq('patient_id', id),
      supabase.from('procedure_orders').update({ is_active: false }).eq('patient_id', id),
      supabase.from('investigation_orders').update({ is_active: false }).eq('patient_id', id),
    ]);
  ...
  async getMedicationOrders(patientId?: string, date?: string): Promise<MedicationOrder[]> {
    let query = supabase.from('medication_orders').select('*').eq('is_active', true);
    
    if (patientId) query = query.eq('patient_id', patientId);
    if (date) query = query.eq('start_date', date);
  ...
  async getProcedureOrders(patientId?: string, date?: string): Promise<ProcedureOrder[]> {
    let query = supabase.from('procedure_orders').select('*').eq('is_active', true);
    
    if (patientId) query = query.eq('patient_id', patientId);
    if (date) query = query.gte('scheduled_time', date).lt('scheduled_time', `${date}T23:59:59`);
  ...
  async getInvestigationOrders(patientId?: string, date?: string): Promise<InvestigationOrder[]> {
    let query = supabase.from('investigation_orders').select('*').eq('is_active', true);
  ...
  async getNursingNotes(patientId?: string): Promise<NursingNote[]> {
    let query = supabase.from('nursing_notes').select('*').order('recorded_at', { ascending: false });
