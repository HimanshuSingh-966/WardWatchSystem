...
export default function InvestigationsPage() {
  ...
  const { data: notifications = [] } = useQuery({
    queryKey: ['/api/notifications/pending', 'investigation'],
    queryFn: async () => {
      const res = await fetch('/api/notifications/pending?type=investigation', {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('auth_token')}` }
      });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Investigation order deleted successfully",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const completeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Investigation order marked as complete",
      });
    },
    onError: (error: Error) => {
      toast({
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 pb-16 px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <h2 className="text-2xl font-semibold text-foreground mb-6">Investigation Orders</h2>
          {timelineLoading ? (
